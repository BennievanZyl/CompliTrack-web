...
export default function DevicesPage() {
  ...
  async function removeDevice(id: string) {
    setRemoving(id)
    try {
      // Try Edge Function first (terminates session on device)
      const { data: { session } } = await supabase.auth.getSession()
      const resp = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/revoke-device`, {
        method: 'POST',
        headers: {
      ...
      // If Edge Function not deployed or fails, fall back to direct DB update
      if (!resp.ok) {
        await supabase.from('device_invitations').update({ used_at: null, device_name: null }).eq('id', id)
      }
    } catch {
      // Fallback: just unlink in DB
      await supabase.from('device_invitations').update({ used_at: null, device_name: null }).eq('id', id)
    } finally {
