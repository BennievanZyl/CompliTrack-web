    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f0', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <Modal show={showAddPurchase} onClose={() => setShowAddPurchase(false)} title="Log Purchase">
        <div style={{ padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div><label style={LABEL}>Stock Item (optional)</label>
            <input type="text" placeholder="Search item..." value={purchaseForm.item_name && !purchaseForm.stock_item_id ? purchaseForm.item_name : ''} onChange={e => { setPurchaseForm(f => ({ ...f, item_name: e.target.value, stock_item_id: '' })) }} style={{ ...INPUT, marginBottom: '6px' }} />
      ...
      <Modal show={showAddWastage} onClose={() => setShowAddWastage(false)} title="Log Wastage">
        <div style={{ padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div><label style={LABEL}>Stock Item</label><select value={wastageForm.stock_item_id} onChange={e => { const item = items.find(i => i.id === e.target.value); setWastageForm(f => ({ ...f, stock_item_id: e.target.value, item_name: (item?.name || item?.description) || '', unit: item?.unit || f.unit, unit_cost: item ? String(Number(item.cost_price) || Number(item.price) || 0) : f.unit_cost })) }} style={INPUT}><option value="">Select stock item</option>{suppliers.map(sup => { const supItems = items.filter(i => (i.supplier || 'Other') === sup.name); return supItems.length ? <optgroup key={sup.id} label={sup.name}>{supItems.map(i => <option key={i.id} value={i.id}>{i.description || i.name}</option>)}</optgroup> : null })}</select></div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
      ...
      <Modal show={showAddOrder} onClose={() => setShowAddOrder(false)} title="New Purchase Order" maxWidth="700px">
        <div style={{ padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {orderForm.supplier_name && (
            <div style={{ background: '#f9fafb', borderRadius: '12px', padding: '16px' }}>
              ...
              {items.filter(i => (i.supplier || 'Other') === orderForm.supplier_name).length === 0 ? (
                <p style={{ color: '#6b7280', fontSize: '13px' }}>No items assigned to this supplier yet. Go to Stock Items to assign items.</p>
              ) : (
                  ...
                  {items.filter(i => (i.supplier || 'Other') === orderForm.supplier_name).map(item => (
                    <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '8px', marginBottom: '8px', alignItems: 'center' }}>
                      ...
                      <input type="number" min="0" step="0.1" placeholder="0"
                        value={(orderForm as {[key:string]:string})[`qty_${item.id}`] || ''}
                        onChange={e => setOrderForm(f => ({ ...f, [`qty_${item.id}`]: e.target.value }))}
                        style={{ padding: '6px 10px', border: '1.5px solid #e5e7eb', borderRadius: '8px', fontSize: '14px', outline: 'none',
                          background: parseFloat((orderForm as {[key:string]:string})[`qty_${item.id}`] || '0') > 0 ? '#f0fdf4' : '#fff',
                          borderColor: parseFloat((orderForm as {[key:string]:string})[`qty_${item.id}`] || '0') > 0 ? '#16a34a' : '#e5e7eb' }} />
                    </div>
                  ))}
                </div>
              )}
          ...
          <div><label style={LABEL}>Notes</label><textarea value={orderForm.notes} onChange={e => setOrderForm(f => ({ ...f, notes: e.target.value }))} placeholder="Special instructions for this order..." style={{ ...INPUT, height: '60px', resize: 'vertical' as const }} /></div>
        </div>
        <div style={{ padding: '16px 28px 24px', display: 'flex', gap: '12px' }}>
          ...
          <button onClick={saveOrder} disabled={saving || !orderForm.supplier_name} style={{ flex: 2, background: !orderForm.supplier_name ? '#d1d5db' : '#1a5c38', color: 'white', border: 'none', borderRadius: '12px', padding: '12px', fontSize: '14px', fontWeight: 800, cursor: 'pointer' }}>
