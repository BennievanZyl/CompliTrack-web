...
export default function StockPage() {
  ...
  async function loadAll(sid?: string) {
    ...
    const [catRes, itemRes, countRes, purchRes, wastRes, ordRes, suppRes, issueRes] = await Promise.all([
      supabase.from('stock_categories').select('*').eq('store_id', effectiveSid).order('sort_order'),
      supabase.from('stock_items').select('*').eq('store_id', effectiveSid).eq('is_active', true).order('sort_order', { nullsFirst: false }),
      supabase.from('stock_counts').select('*').eq('store_id', effectiveSid).order('count_date', { ascending: false }).limit(30),
      supabase.from('stock_purchases').select('*').eq('store_id', effectiveSid).order('purchase_date', { ascending: false }).limit(50),
      supabase.from('stock_wastage').select('*').eq('store_id', effectiveSid).order('wastage_date', { ascending: false }).limit(50),
      supabase.from('stock_orders').select('*').eq('store_id', effectiveSid).order('order_date', { ascending: false }).limit(30),
      supabase.from('stock_suppliers').select('*').eq('store_id', effectiveSid).eq('is_active', true).order('sort_order'),
      supabase.from('stock_issues').select('*').eq('store_id', effectiveSid).order('issue_date', { ascending: false }).limit(50),
    ])
  ...
  async function saveSupplier() {
    ...
    setSaving(true)
    const payload = {
      store_id: effectiveSid, ...supplierForm,
  ...
  async function startCount(type: string) {
    ...
    try {
      ...
      const { data: session, error: sessionErr } = await supabase
        .from('stock_counts')
        .insert({ store_id: effectiveSid, count_date: today, count_type: type, status: 'in_progress' })
        .select().single()
  ...
  async function saveIssue() {
    ...
    await supabase.from('stock_issues').insert({
      store_id: effectiveSid, issue_date: new Date().toISOString(), stock_item_id: issueForm.stock_item_id || null,
  ...
  async function saveOrder() {
    ...
    setSaving(true)
    await supabase.from('stock_orders').insert({ store_id: effectiveSid, supplier_name: orderForm.supplier_name, order_date: orderForm.order_date, expected_delivery: orderForm.expected_delivery || null, notes: orderForm.notes || null, status: 'pending', total_value: 0 })
  ...
  async function saveAdjustment() {
    ...
    await supabase.from('stock_adjustments').insert({ store_id: effectiveSid, stock_item_id: adjustItem.id, item_name: adjustItem.description || adjustItem.name, qty_before: qtyBefore, qty_after: qtyAfter, adjustment, unit: adjustItem.unit, reason: adjustReason, notes: adjustNotes })
  ...
  async function saveCategory() {
    ...
    await supabase.from('stock_categories').insert({ store_id: effectiveSid, name: categoryForm.name, color: categoryForm.color, sort_order: categories.length + 1 })
  ...
  async function saveInlineCategory() {
    ...
    const { data: newCat } = await supabase.from('stock_categories')
      .insert({ store_id: effectiveSid, name: newCatName.trim(), color: '#1a5c38', sort_order: categories.length + 1 })
      .select().single()
    await loadAll()
