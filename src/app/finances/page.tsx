...
export default function FinancesPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [cuRes, invRes, catRes, suppRes, qRes, stockRes] = await Promise.all([
      ...
      supabase.from('stock_suppliers').select('id, name, payment_terms_days, invoice_columns, invoice_vat_included, delivers_stock').eq('store_id', storeId).eq('is_active', true).order('sort_order'),
      ...
      supabase.from('stock_items')
        ...
        .eq('store_id', storeId).eq('is_active', true).order('description'),
    ])
  ...
  async function loadFoodCost() {
    ...
    const { data: counts } = await supabase.from('stock_counts')
      .select('id, count_date, status').eq('store_id', storeId).eq('status', 'completed')
    ...
    const { data: overrides } = await supabase.from('food_cost_overrides').select('*')
      .eq('store_id', storeId).eq('period_start', periodStart).eq('period_end', periodEnd)
    ...
    const [purchRes, wasteRes, salesRes, recentOverridesRes] = await Promise.all([
      supabase.from('stock_purchases').select('total_cost').eq('store_id', storeId).gte('purchase_date', periodStart).lte('purchase_date', periodEnd),
      supabase.from('stock_wastage').select('total_cost').eq('store_id', storeId).gte('wastage_date', periodStart).lte('wastage_date', periodEnd),
      supabase.from('cash_ups').select('cash_up_total').eq('store_id', storeId).not('status', 'eq', 'draft').gte('cash_up_date', periodStart).lte('cash_up_date', periodEnd),
      supabase.from('food_cost_overrides').select('id', { count: 'exact', head: true }).eq('store_id', storeId).gte('created_at', new Date(Date.now() - 90 * 86400000).toISOString()),
    ])
  ...
  async function saveFoodCostOverride() {
    ...
    await supabase.from('food_cost_overrides').upsert({
      store_id: storeId, period_start: periodStart, period_end: periodEnd,
  ...
  async function clearFoodCostOverride(field: 'opening' | 'closing') {
    ...
    await supabase.from('food_cost_overrides').delete()
      .eq('store_id', storeId).eq('period_start', periodStart).eq('period_end', periodEnd).eq('field', field)
    await loadFoodCost()
  }
  ...
  async function saveQuick() {
    ...
    const payload = {
      store_id: storeId, expense_date: qForm.expense_date,
