...
export default function ReportsPage() {
  ...
  async function salesReport(fmt_: 'csv' | 'excel' | 'pdf') {
    ...
    const rows = (data || []).map(r => ({ Date: r.cash_up_date, 'Sales (incl VAT)': Number(r.cash_up_total || 0).toFixed(2), 'Sales (ex-VAT)': (Number(r.cash_up_total || 0) / 1.15).toFixed(2), 'VAT (15%)': (Number(r.cash_up_total || 0) * 0.15 / 1.15).toFixed(2), 'A Total': Number(r.a_total || 0).toFixed(2), 'Bank': Number(r.bank_total || 0).toFixed(2), 'EFT': Number(r.eft_total || 0).toFixed(2), Variance: Number(r.variance || 0).toFixed(2), 'Signed By': r.signed_by_name || '', Status: r.status || '', Notes: r.notes || '' }))
    const total = (data || []).reduce((s, r) => s + Number(r.cash_up_total || 0), 0)
    const totals = { Date: 'TOTAL', 'Sales (incl VAT)': total.toFixed(2), 'Sales (ex-VAT)': (total / 1.15).toFixed(2), 'VAT (15%)': (total * 0.15 / 1.15).toFixed(2), 'A Total': '', 'Bank': '', 'EFT': '', Variance: '', 'Signed By': '', Status: '', Notes: '' }
  ...
  async function expensesReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const [{ data: exp }, { data: inv }] = await Promise.all([
      supabase.from('expenses').select('expense_date,category_name,description,amount,vat_amount,supplier,invoice_number,payment_method').eq('store_id', STORE_ID).gte('expense_date', startDate).lte('expense_date', endDate).order('expense_date'),
      supabase.from('invoices').select('invoice_date,supplier,invoice_number,total_amount,total_vat,status,payment_method').eq('store_id', STORE_ID).in('status', ['received', 'paid']).gte('invoice_date', startDate).lte('invoice_date', endDate).order('invoice_date'),
    ])
  ...
  async function invoiceReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data: invs } = await supabase.from('invoices').select('invoice_date,due_date,supplier,invoice_number,total_amount,total_vat,status,payment_method,notes').eq('store_id', STORE_ID).gte('invoice_date', startDate).lte('invoice_date', endDate).order('invoice_date')
  ...
  async function outstandingBillsReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('invoices').select('invoice_date,due_date,supplier,invoice_number,total_amount,notes').eq('store_id', STORE_ID).eq('status', 'received').order('due_date', { ascending: true, nullsFirst: false })
    const today_ = today()
  ...
  async function payrollReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('wage_payments').select('paid_date,payment_method,gross_pay,net_pay,uif_employee,uif_employer,employee_id,period').eq('store_id', STORE_ID).gte('paid_date', startDate).lte('paid_date', endDate).order('paid_date')
  ...
  async function attendanceReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('attendance').select('work_date,clock_in,clock_out,hours_worked,is_late,employee_id').eq('store_id', STORE_ID).gte('work_date', startDate).lte('work_date', endDate).order('work_date')
    const empIds = [...new Set((data || []).map(r => r.employee_id).filter(Boolean))]
  ...
  async function uifReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('wage_payments').select('period,gross_pay,uif_employee,uif_employer,employee_id').eq('store_id', STORE_ID).gte('paid_date', startDate).lte('paid_date', endDate)
    const empIds = [...new Set((data || []).map(r => r.employee_id).filter(Boolean))]
  ...
  async function stockValuationReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('stock_items').select('name,description,unit,current_qty,cost_price,price,supplier,category').eq('store_id', STORE_ID).eq('is_active', true).order('name')
  ...
  async function purchasesReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('stock_purchases').select('purchase_date,stock_item_id,item_name,qty_received,unit,unit_cost,total_cost,supplier').eq('store_id', STORE_ID).gte('purchase_date', startDate).lte('purchase_date', endDate).order('purchase_date')
  ...
  async function wastageReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('stock_wastage').select('wastage_date,item_name,qty,unit,unit_cost,total_cost,reason,recorded_by').eq('store_id', STORE_ID).gte('wastage_date', startDate).lte('wastage_date', endDate).order('wastage_date')
  ...
  async function stockMovementReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const [{ data: purchases }, { data: issues }, { data: wastage }] = await Promise.all([
      supabase.from('stock_purchases').select('purchase_date,item_name,qty_received,unit,total_cost').eq('store_id', STORE_ID).gte('purchase_date', startDate).lte('purchase_date', endDate),
      supabase.from('stock_issues').select('issue_date,item_name,quantity,unit,issued_to').eq('store_id', STORE_ID).gte('issue_date', startDate).lte('issue_date', endDate),
      supabase.from('stock_wastage').select('wastage_date,item_name,qty,unit,total_cost').eq('store_id', STORE_ID).gte('wastage_date', startDate).lte('wastage_date', endDate),
    ])
  ...
  async function complianceReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data: sessions } = await supabase.from('daily_sessions').select('id,session_date,status,duration_seconds,session_type').eq('store_id', STORE_ID).eq('session_type', 'daily').gte('session_date', startDate).lte('session_date', endDate).order('session_date')
  ...
  async function temperatureReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('temperature_logs').select('logged_at,equipment_name,temperature,min_temp,max_temp,is_compliant,notes,logged_by').eq('store_id', STORE_ID).gte('logged_at', startDate).lte('logged_at', endDate + 'T23:59:59').order('logged_at')
  ...
  async function employeeListReport(fmt_: 'csv' | 'excel' | 'pdf') {
    const { data } = await supabase.from('employees').select('full_name,id_number,role,phone,email,hourly_rate,pay_frequency,is_active').eq('store_id', STORE_ID).order('full_name')
  ...
  async function accountantPack() {
    const [salesRes, expRes, invRes, payrollRes, purchRes, wastRes] = await Promise.all([
      supabase.from('cash_ups').select('cash_up_date,cash_up_total,a_total,bank_total,eft_total,variance,signed_by_name').eq('store_id', STORE_ID).gte('cash_up_date', startDate).lte('cash_up_date', endDate).order('cash_up_date'),
      ...
      supabase.from('stock_purchases').select('purchase_date,item_name,qty_received,unit,unit_cost,total_cost,supplier').eq('store_id', STORE_ID).gte('purchase_date', startDate).lte('purchase_date', endDate).order('purchase_date'),
      supabase.from('stock_wastage').select('wastage_date,item_name,qty,unit,total_cost,reason').eq('store_id', STORE_ID).gte('wastage_date', startDate).lte('wastage_date', endDate).order('wastage_date'),
    ])
