...
export default function ReportsPage() {
  ...
  async function loadReports() {
    ...
    const { data: sessions } = await supabase
      .from('daily_sessions')
      .select('id, session_date, status, signed_off_at, pdf_report_url, started_at, duration_seconds')
    ...
    const enriched = await Promise.all(sessions.map(async (s) => {
      ...
      return { ...s, total: total || 0, completed: completed || 0, uniform_photos: uniform_photos || 0, temp_violations: temp_violations || 0 };
    }));
  ...
  const avgDuration = reports.filter(r => r.duration_seconds).length > 0
    ? Math.round(reports.filter(r => r.duration_seconds).reduce((sum, r) => sum + (r.duration_seconds || 0), 0) / reports.filter(r => r.duration_seconds).length)
    : 0;
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 20, marginBottom: 32 }}>
              ...
              <div style={cardStyle}>
                ...
                <div style={{ fontSize: 28, fontWeight: 800, color: PRIMARY }}>{avgDuration > 0 ? formatDuration(avgDuration) : '-'}</div>
                <div style={{ fontSize: 13, color: '#888', marginTop: 4 }}>per compliance session</div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20, marginBottom: 32 }}>
                      ...
                      {last7.map((r) => {
                        ...
                        return (
                          <div key={r.id} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                            ...
                            <span style={{ fontSize: 10, color: '#aaa', textAlign: 'center' }}>{new Date(r.session_date).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' })}</span>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={cardStyle}>
                  ...
                  {reports.filter(r => r.duration_seconds).length === 0 ? (
                    <div style={{ color: '#aaa', fontSize: 13 }}>No timed sessions yet</div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {[
                        { label: 'Fastest', session: reports.filter(r => r.duration_seconds).reduce((a, b) => (a.duration_seconds || 0) < (b.duration_seconds || 0) ? a : b), color: PRIMARY },
                        { label: 'Slowest', session: reports.filter(r => r.duration_seconds).reduce((a, b) => (a.duration_seconds || 0) > (b.duration_seconds || 0) ? a : b), color: '#f59e0b' },
                  ...
                  <div key={report.id} onClick={() => router.push('/compliance')} style={{ padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 20, borderBottom: idx < filtered.length - 1 ? '1px solid #f0f4f0' : 'none', cursor: 'pointer' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                        ...
                        {report.duration_seconds && <span style={{ fontSize: 12, color: PRIMARY, fontWeight: 600 }}>{formatDuration(report.duration_seconds)}</span>}
                      </div>
                    </div>
