...
export default function ClockKioskPage() {
  ...
  async function loadFaceApi() {
    ...
    // Already loaded
    if ((window as unknown as { faceapi: FaceApi }).faceapi) {
      ...
      try {
        ...
        if (detection) {
          const labeledDescriptors = employeesWithFaces.map(e =>
            new faceapi.LabeledFaceDescriptors(e.id, [new Float32Array(e.face_descriptor!)])
          );
  ...
  async function enrollFace() {
    ...
    if (!faceApiLoaded) { setEnrollError('Face recognition not ready yet. Please wait or use PIN mode.'); return; }
    ...
    try {
      ...
      const detection = await faceapi.detectSingleFace(videoRef.current, new faceapi.TinyFaceDetectorOptions())
        ...
        .withFaceDescriptor();
      if (!detection) {
        setEnrollError('No face detected. Please look directly at the camera and try again.');
    ...
    <div style={{ minHeight: '100vh', background: DARK, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui, sans-serif', padding: 24 }}>
      ...
      <div style={{ position: 'fixed' as const, top: 0, left: 0, right: 0, padding: '16px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button
            onClick={() => { setMode('enrolling'); setEnrollStep('select'); setEnrollEmployee(null); setEnrollError(''); }}
            style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: 'rgba(255,255,255,0.7)', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 12 }}
          >
            👤 Enroll Face
          </button>
          <button
      ...
      {mode === 'scanning' && (
        <div style={{ position: 'relative' as const, zIndex: 1, textAlign: 'center' as const, maxWidth: 480, width: '100%' }}>
          <div style={{ width: 200, height: 200, borderRadius: '50%', border: '4px solid #4ade80', margin: '0 auto 32px', position: 'relative' as const, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {enrollStep === 'select' && (
            <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 20, padding: 24 }}>
              <input
                ...
                style={{ width: '100%', padding: '12px 16px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, outline: 'none', marginBottom: 16, boxSizing: 'border-box' as const }}
              />
              <div style={{ maxHeight: 280, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                {filteredEnrollEmployees.map(emp => {
                  ...
                  return (
                    <div key={emp.id} onClick={() => setEnrollEmployee(emp)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: enrollEmployee?.id === emp.id ? 'rgba(26,92,56,0.5)' : 'rgba(255,255,255,0.05)', borderRadius: 12, padding: '12px 16px', cursor: 'pointer', border: `1.5px solid ${enrollEmployee?.id === emp.id ? PRIMARY : 'rgba(255,255,255,0.1)'}`, transition: 'all 0.15s' }}>
              ...
              {enrollError && (
                <div style={{ background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: 10, padding: '10px 14px', marginBottom: 14, color: '#fca5a5', fontSize: 13, fontWeight: 600, textAlign: 'left' as const }}>
                  {enrollError}
                </div>
              )}
              ...
              <div style={{ marginBottom: 16, fontSize: 12, textAlign: 'left' as const }}>
                ...
                {cameraReady && !faceApiLoaded && !loadingModels && <div style={{ color: 'rgba(239,68,68,0.7)' }}>⚠️ Face recognition not available — check /public/models/ files</div>}
                {cameraReady && faceApiLoaded && <div style={{ color: 'rgba(74,222,128,0.7)' }}>✅ Camera ready · Face recognition ready</div>}
              </div>

              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => { setMode('idle'); setEnrollEmployee(null); setEnrollSearch(''); setEnrollError(''); }} style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', borderRadius: 12, cursor: 'pointer', fontWeight: 600 }}>
                  Cancel
                </button>
                <button
                  onClick={enrollFace}
                  disabled={!enrollEmployee}
                  style={{ flex: 2, padding: '12px', background: enrollEmployee ? PRIMARY : 'rgba(255,255,255,0.1)', color: enrollEmployee ? '#fff' : 'rgba(255,255,255,0.3)', border: 'none', borderRadius: 12, cursor: enrollEmployee ? 'pointer' : 'not-allowed', fontWeight: 700, fontSize: 15, transition: 'all 0.2s' }}
                >
                  📸 Capture Face
                </button>
              </div>
